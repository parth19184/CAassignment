...
class L1Cache :
    def __init__(self, mem, size = 64, miss_penalty = 2, hit_time = 1, block_size = 4, assoc = 4, write_pol = 0,replacement_pol = 0):
        ...
        self.write_policy = write_pol                           # 0: Write Back, 1: Write through
        self.replacement_pol = replacement_pol                  # 0: LRU, 1: FIFO, 3: Random
        ...
        self.FIFO = np.zeros((self.sets,self.assoc))            # for FIFO policy
    ...
    def replace(self,index,offset):
        ...
        if(self.replacement_pol == 0):
            ...
            det = np.argmax(self.FIFO[index])   # FIFO replacement
        else:
            det = random.randint(0,3)           # random replacement
...
class Mem : 
    def __init__(self, size = 1024, access_time = 4):
        self.bits = 32
        self.size = size
